...
import { getForumCategoriesWithForums, type ForumCategory, type ForumForum } from '../../lib/forum';
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        <header className="rounded-2xl border border-green-500/20 bg-zinc-900/60 p-6 sm:p-10 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative space-y-4">
            <div className="inline-flex items-center gap-2 px-3 py-2 rounded-full bg-green-500/10 text-green-300 text-sm font-semibold">
        ...
        <div className="grid gap-6">
          {loading ? (
            ...
            categories.map((category) => (
              <article
                key={category.id}
                className="rounded-2xl border border-white/10 bg-white/5 p-6 sm:p-7 shadow-xl"
              >
                ...
                <div className="mt-5 grid md:grid-cols-2 gap-3">
                  {category.forums.map((forum) => (
                    <button
                      ...
                      className="w-full text-left rounded-xl border border-white/10 bg-zinc-900/60 hover:border-green-400/60 hover:bg-zinc-900/90 transition p-4"
                    >
